...
SchedTCB_t *pxThisTask = ( SchedTCB_t * ) pvTaskGetThreadLocalStoragePointer( xTaskGetCurrentTaskHandle(), schedTHREAD_LOCAL_STORAGE_POINTER_INDEX );
	...
	for( ; ; )
	{
		...
		/*
		taskENTER_CRITICAL(&ESFree_schedulerMutex);
		...
		taskEXIT_CRITICAL(&ESFree_schedulerMutex);
		*/
...
SchedTCB_t *pxShortestTaskPointer, *pxTCB;
	...
	// error: 'pxShortestTaskPointer' may be used uninitialized in this function [-Werror=maybe-uninitialized]
	pxShortestTaskPointer = NULL;
	...
	for( xIter = 0; xIter < xTaskCounter; xIter++ )
	{
		...
		if (pxShortestTaskPointer){ // error: 'pxShortestTaskPointer' may be used uninitialized in this function [-Werror=maybe-uninitialized]
